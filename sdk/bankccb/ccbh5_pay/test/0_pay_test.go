...
import (
	...
	"github.com/zingson/go-helper/sdk/bankccb/ccbh5_pay"
	"testing"
)

func getConfig() (conf *ccbh5_pay.Config) {
...
func TestSvcOccPlatOrderQry(t *testing.T) {
	conf := getConfig()
	_, err := ccbh5_pay.Query(conf, &ccbh5_pay.QryParams{
		TX_TYPE:             ccbh5_pay.TX_TYPE_0,
		TXN_PRD_TPCD:        "99",
...
func TestSvcOccPlatOrderQry2(t *testing.T) {
	conf := getConfig()
	cldBody, err := ccbh5_pay.Query(conf, &ccbh5_pay.QryParams{
		TX_TYPE:             ccbh5_pay.TX_TYPE_0,
		TXN_PRD_TPCD:        "06",
		STDT_TM:             "",
		EDDT_TM:             "",
...
func TestRefund(t *testing.T) {
	conf := getConfig()
	cldBody, err := ccbh5_pay.Refund(conf, "15155562905736929293", 1, "20220417142653")
	if err != nil {
		t.Error(err)
		return
	}
