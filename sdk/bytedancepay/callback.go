...
// out  输出
func Callback(token, mchid, uri string, reqBody string, resolve func(notify *Notify) error) (resBody string) {
	var err error
	var notify *Notify
	defer func() {
		errMsg := ""
		...
		logrus.WithField("mchid", mchid).Infof("ZJ 支付回调通知 POST %s  请求报文：%s  响应报文：%s  %s ", uri, reqBody, resBody, errMsg)
	}()
...
type PaymentNotify struct {
	...
	Status         string `json:"status"`           //PROCESSING-处理中|SUCCESS-成功|FAIL-失败|TIMEOUT-超时
