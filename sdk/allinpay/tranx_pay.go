...
//Pay 统一支付下单接口 , 注意：根据trxstatus判断状态   文档：https://aipboss.allinpay.com/know/devhelp/main.php?pid=15#mid=88
...
type PayParams struct {
	...
	Remark        string  `json:"remark"`        //备注信息 最大160个字节(80个中文字符)，不能出现符号
	Validtime     string  `json:"validtime"`     //订单有效时间，以分为单位，不填默认为5分钟 默认5分钟，最大1440分钟
	Acct          string  `json:"acct"`          //支付平台用户标识 JS支付时使用 微信支付-用户的微信openid  支付宝支付-用户user_id  微信小程序-用户小程序的openid  云闪付JS-用户userId
	NotifyUrl     string  `json:"notify_url"`    //交易结果通知地址
	LimitPay      string  `json:"limit_pay"`     //支付限制
	SubAppid      string  `json:"sub_appid"`     //微信小程序/微信公众号/APP的appid
	GoodsTag      string  `json:"goods_tag"`     //订单优惠标识 只对微信支付有效 W01交易方式不支持
