...
func FrontToken(upConf *upapi.Config) string {
	frontToken, _ := upapi.FrontToken(upConf)
	return frontToken.FrontToken
}
...
func main() {
	...
	http.HandleFunc("/consume", func(writer http.ResponseWriter, request *http.Request) {
		...
		bm := upapi.UpsdkConfig(upConf, "https://ms.himkt.cn/mswork"+request.RequestURI, true, FrontToken)
